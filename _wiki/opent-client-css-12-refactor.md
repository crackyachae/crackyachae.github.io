...
## Link와 Import
동일한 css가 적용된 웹 페이지가 여러개일 때, 수정사항을 일괄적으로 적용할 수 있도록 해줌
...
+ `.styl` 파일명 앞에 -w 옵션을 넣으면(watch를 의미) `style.styl`의 변경사항을 저장할 때마다 자동으로 `style.css`파일을 compile 한다.
  ```
