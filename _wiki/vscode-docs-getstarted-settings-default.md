---
layout  : wiki
title   : GET STARTED_Default Settings
summary : 쉽게 찾아보려고 정리해둔 VS Code 기본 세팅
date    : 2020-03-15 23:08:48 +0900
updated : 2021-02-07 22:32:59 +0900
...
> 이 글은 Visual Studio Code Documentation [User and Workspace Settings #Default Settings](https://code.visualstudio.com/docs/getstarted/settings#_default-settings) 를 읽으면서 개인적으로 번역한 글입니다.
>
...
```
+ 사용자가 여는 괄호를 입력했을 때 에디터가 자동으로 괄호를 닫을 것인지를 결정한다.
  + `always`
  + `languageDefined`: 자동으로 괄호를 닫을 것인지를 결정하는데 언어별 설정을 사용한다.
  + `beforeWhitespace`: 커서가 화이트 스페이스(whitespace)의 왼쪽에 있을 때만 괄호를 자동으로 닫는다.
  + `never`

```json
"editor.autoClosingOvertype": "auto"
```
+ 에디터가 닫는 따옴표나 괄호를 덮어쓸 것인지를 결정한다.
  + `always`
  + `auto`: 닫는 따옴표나 괄호가 자동으로 삽입된 것이었을때만 기존에 적혀있던 닫는 따옴표나 괄호를 덮어쓴다(type over).
  + `never`

```json
"editor.autoClosingQuotes": "languageDefined"
```
+ 사용자가 여는 따옴표 입력했을 때 에디터가 자동으로 따옴표를 닫을 것인지를 결정한다.
  + `always`
  + `languageDefined`: 자동으로 따옴표를 닫을 것인지를 결정하는데 언어별 설정을 사용한다.
  + `beforeWhitespace`: 커서가 화이트 스페이스(whitespace)의 왼쪽에 있을 때만 따옴표를 자동으로 닫는다.
  + `never`
...
```
+ 다른 모든 formatter 설정보다 우선되는 기본 formatter를 정의한다.
+ 반드시 formatter를 제공하는 확장 프로그램의 식별자(identifier)여야 한다.
...
```
+ JSX 태그를 자동으로 닫힘을 활성화/비활성화한다.
+ 작업영역에서 TypeScript 3.0 이상의 버전을 사용해야한다. 

### Format
다른 default format 설정이 없을 때만 유효한 듯 하다.

```json  
  "javascript.format.enable": true,
```
+ 기본 자바스크립트 formatter를 활성화/비활성화 한다.
+ 비활성화하면 formatOnSave 설정 후 저장해도 formatting이 되지않는 것을 확인할 수 있다.
