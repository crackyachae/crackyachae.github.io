...
IS_CORRECT_WORKING_DIRECTORY=`pwd | egrep "crackyachae.github.io$" | wc -l`
if (( "$IS_CORRECT_WORKING_DIRECTORY" != 1 )); then
    echo "올바른 경로가 아닙니다. crackyachae.github.io/ 에서 실행해 주세요."
    exit;
fi
...
for RESOURCE_FILE_ADDRESS in $RES_LIST; do
    ...
    # [TODO] 대상에 diary, project, pscode 추가
    REFERENCE_LIST=`ag -l "$RESOURCE_FILE_URL" _study | egrep "\.md"`
